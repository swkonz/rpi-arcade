...
static void draw_pipe(int location, int x, int height) {
    // Draw long body
    int y = (location == PIPE_NORTH) ? 0 : HEIGHT - height;
...
static void calculate_heights(pipe_t* pipe) {
	pipe->north_height = (HEIGHT - PIPE_TOP_HEIGHT*2 - PIPE_GAP) / 2 + pipe->y_offset;
	pipe->south_height = (HEIGHT - PIPE_TOP_HEIGHT*2 - PIPE_GAP) / 2 - pipe->y_offset;
}
